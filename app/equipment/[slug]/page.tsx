...
export async function generateMetadata({
  ...
  if (!eq) {
    return constructMetadata({
      title: 'Aquarium Equipment Hardware Guide',
      description: 'Aquarium hardware guide, filtration sizing, and equipment maintenance.',
      path: `/equipment/${slug}`,
    });
  }

  const titleText = `${eq.name} Guide: Sizing, Purpose & Maintenance`;
  const descText = `Aquarium hardware specifications and guide for ${eq.name}. Learn its biological purpose, operational mechanics, maintenance protocols, and setup recommendations.`;
...
export default async function EquipmentDetailPage({
  ...
  const breadcrumbs = constructBreadcrumbSchema([
    ...
    { name: eq.name, url: `/equipment/${eq.slug}` },
  ]);
  ...
  const articleSchema = {
    ...
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": `${siteConfig.url}/equipment/${eq.slug}`
    }
  };
        ...
        <div className="site-container">
          ...
          <nav aria-label="Breadcrumb" className="mb-6">
            <ol className="flex flex-wrap items-center gap-2 text-xs sm:text-sm font-semibold text-[#27187e]/70">
              ...
              <li>•</li>
              <li>
                <Link href="/equipment" className="hover:text-[#27187e] hover:underline">Equipment Guide</Link>
              </li>
              <li>•</li>
              <li className="text-[#27187e] font-bold" aria-current="page">{eq.name}</li>
            </ol>
          </nav>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-8 space-y-8">
              ...
              <div className="bg-[#ffffff] border-2 border-[#cfcaf5] rounded-3xl p-7 sm:p-8 shadow-sm">
                ...
                <p className="text-base sm:text-lg text-[#27187e]/90 leading-relaxed">
                  {eq.purpose}
                </p>
              </div>
